# System Architecture Overview

## High-Level Architecture

```
┌─────────────────────────────────────────────────────────────────┐
│                         END USERS                                │
│                                                                  │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐         │
│  │   Customer   │  │    Admin     │  │  Embedded    │         │
│  │     Chat     │  │  Dashboard   │  │   Widget     │         │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘         │
│         │                  │                  │                  │
└─────────┼──────────────────┼──────────────────┼─────────────────┘
          │                  │                  │
          │       ┌──────────┴──────────┐      │
          │       │                     │      │
          └───────►   FRONTEND LAYER    ◄──────┘
                  │  (HTML/CSS/JS)      │
                  │                     │
                  └──────────┬──────────┘
                             │
                    REST API / WebSocket
                             │
          ┌──────────────────┴──────────────────┐
          │                                      │
          │        BACKEND API LAYER             │
          │         (Flask + Python)             │
          │                                      │
          │  ┌────────────────────────────┐    │
          │  │     Authentication         │    │
          │  │  (JWT, API Keys)           │    │
          │  └────────────────────────────┘    │
          │                                      │
          │  ┌────────────────────────────┐    │
          │  │    Business Logic          │    │
          │  │  (Services Layer)          │    │
          │  └────────────────────────────┘    │
          │                                      │
          └──────────┬───────────────────────────┘
                     │
          ┌──────────┴───────────┐
          │                      │
    ┌─────▼──────┐        ┌─────▼──────┐
    │   AI/ML    │        │  Database  │
    │   Layer    │        │   Layer    │
    └────────────┘        └────────────┘
```

---

## Detailed Component Architecture

### 1. Frontend Layer

```
┌─────────────────────────────────────────────────────────┐
│                    FRONTEND                             │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  ┌──────────────────────────────────────────────┐      │
│  │         Public Pages                         │      │
│  │  • Landing page (index.html)                 │      │
│  │  • Pricing (pricing.html)                    │      │
│  │  • Login/Signup (auth.html)                  │      │
│  └──────────────────────────────────────────────┘      │
│                                                          │
│  ┌──────────────────────────────────────────────┐      │
│  │      Protected Dashboard                     │      │
│  │  • Overview (dashboard.html)                 │      │
│  │  • Conversations (conversations.html)        │      │
│  │  • Documents (documents.html)                │      │
│  │  • Analytics (analytics.html)                │      │
│  │  • Settings (settings.html)                  │      │
│  └──────────────────────────────────────────────┘      │
│                                                          │
│  ┌──────────────────────────────────────────────┐      │
│  │      Embeddable Widget                       │      │
│  │  • chat-widget.js (main)                     │      │
│  │  • widget.css (styling)                      │      │
│  │  • WebSocket client                          │      │
│  └──────────────────────────────────────────────┘      │
│                                                          │
│  ┌──────────────────────────────────────────────┐      │
│  │         Shared Components                    │      │
│  │  • API client (api.js)                       │      │
│  │  • Auth manager (auth.js)                    │      │
│  │  • Utilities (utils.js)                      │      │
│  │  • Chart library (Chart.js)                  │      │
│  └──────────────────────────────────────────────┘      │
└─────────────────────────────────────────────────────────┘
```

---

### 2. Backend API Layer

```
┌─────────────────────────────────────────────────────────┐
│                  BACKEND (Flask)                        │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  app/                                                    │
│  │                                                       │
│  ├── api/ (Routes/Endpoints)                           │
│  │   ├── auth.py           # POST /api/auth/login      │
│  │   ├── chat.py           # WS   /api/chat/ws         │
│  │   ├── admin.py          # GET  /api/admin/stats     │
│  │   ├── documents.py      # POST /api/documents       │
│  │   ├── payments.py       # POST /api/payments/webhook│
│  │   └── analytics.py      # GET  /api/analytics       │
│  │                                                       │
│  ├── services/ (Business Logic)                        │
│  │   ├── embedding_service.py    # OpenAI embeddings  │
│  │   ├── vector_store_service.py # ChromaDB ops       │
│  │   ├── sentiment_service.py    # Sentiment analysis │
│  │   ├── chat_service.py         # RAG + LLM          │
│  │   ├── auth_service.py         # Auth logic         │
│  │   ├── ticket_service.py       # Ticket management  │
│  │   └── payment_service.py      # Stripe integration │
│  │                                                       │
│  ├── models/ (Database Models)                         │
│  │   ├── user.py                  # User table         │
│  │   ├── organization.py          # Org table          │
│  │   ├── conversation.py          # Conversation table │
│  │   ├── message.py               # Message table      │
│  │   ├── ticket.py                # Ticket table       │
│  │   └── subscription.py          # Subscription table │
│  │                                                       │
│  └── utils/ (Helper Functions)                         │
│      ├── jwt_helper.py            # Token management   │
│      ├── validators.py            # Input validation   │
│      └── rate_limiter.py          # Rate limiting      │
│                                                          │
│  config.py               # Configuration                │
│  run.py                  # Application entry            │
└─────────────────────────────────────────────────────────┘
```

---

### 3. AI/ML Layer

```
┌─────────────────────────────────────────────────────────┐
│                    AI/ML PIPELINE                        │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  Step 1: User Message Arrives                          │
│     │                                                    │
│     ├──► Sentiment Analysis                            │
│     │    (transformers)                                 │
│     │    → Detect emotion                               │
│     │    → Calculate priority                           │
│     │    → Flag escalation                              │
│     │                                                    │
│     ├──► Generate Embedding                             │
│     │    (OpenAI text-embedding-3-small)                │
│     │    → Convert to 1536-dim vector                   │
│     │                                                    │
│     └──► Vector Search                                  │
│          (ChromaDB)                                      │
│          → Find relevant documents                       │
│          → Rank by similarity                            │
│          → Return top 3 contexts                         │
│                                                          │
│  Step 2: Context Preparation                           │
│     │                                                    │
│     ├──► Retrieve Conversation History                 │
│     │    (Last 5 messages)                              │
│     │                                                    │
│     └──► Combine:                                       │
│          • Retrieved documents                           │
│          • Conversation history                          │
│          • User message                                  │
│          • System prompt                                 │
│                                                          │
│  Step 3: LLM Generation                                │
│     │                                                    │
│     └──► LangChain RAG Chain                           │
│          (GPT-4-turbo or GPT-3.5-turbo)                │
│          → Generate contextual response                  │
│          → Cite sources                                  │
│          → Handle edge cases                             │
│                                                          │
│  Step 4: Post-Processing                               │
│     │                                                    │
│     ├──► Quality Check                                  │
│     │    → Validate response                            │
│     │    → Check for hallucinations                     │
│     │                                                    │
│     ├──► Format Response                                │
│     │    → Add citations                                │
│     │    → Apply markdown                               │
│     │                                                    │
│     └──► Log & Store                                    │
│          → Save to database                              │
│          → Update analytics                              │
│                                                          │
│  Step 5: Return to User                                │
│     Response with metadata                               │
│     • Message text                                       │
│     • Sentiment score                                    │
│     • Confidence level                                   │
│     • Suggested actions                                  │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

---

### 4. Database Layer

```
┌─────────────────────────────────────────────────────────┐
│                  DATABASE SCHEMA                         │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  SQLite (Dev) / PostgreSQL (Prod)                      │
│                                                          │
│  ┌──────────────────┐                                   │
│  │  organizations   │                                   │
│  ├──────────────────┤                                   │
│  │ id               │                                   │
│  │ name             │                                   │
│  │ api_key          │                                   │
│  │ created_at       │                                   │
│  └────────┬─────────┘                                   │
│           │ 1                                            │
│           │                                              │
│           │ *                                            │
│  ┌────────▼─────────┐                                   │
│  │  users           │                                   │
│  ├──────────────────┤                                   │
│  │ id               │                                   │
│  │ email            │                                   │
│  │ password_hash    │                                   │
│  │ organization_id  │◄─────┐                           │
│  │ role             │       │                           │
│  │ created_at       │       │                           │
│  └────────┬─────────┘       │                           │
│           │ 1                │                           │
│           │                  │                           │
│           │ *                │                           │
│  ┌────────▼─────────┐       │                           │
│  │  conversations   │       │                           │
│  ├──────────────────┤       │                           │
│  │ id               │       │                           │
│  │ organization_id  │───────┘                           │
│  │ customer_email   │                                   │
│  │ status           │                                   │
│  │ priority         │                                   │
│  │ created_at       │                                   │
│  └────────┬─────────┘                                   │
│           │ 1                                            │
│           │                                              │
│           │ *                                            │
│  ┌────────▼─────────┐                                   │
│  │  messages        │                                   │
│  ├──────────────────┤                                   │
│  │ id               │                                   │
│  │ conversation_id  │                                   │
│  │ content          │                                   │
│  │ role (user/ai)   │                                   │
│  │ sentiment_score  │                                   │
│  │ created_at       │                                   │
│  └──────────────────┘                                   │
│                                                          │
│  ┌──────────────────┐                                   │
│  │  tickets         │                                   │
│  ├──────────────────┤                                   │
│  │ id               │                                   │
│  │ conversation_id  │                                   │
│  │ category         │                                   │
│  │ status           │                                   │
│  │ assigned_to      │                                   │
│  │ created_at       │                                   │
│  └──────────────────┘                                   │
│                                                          │
│  ┌──────────────────┐                                   │
│  │  documents       │                                   │
│  ├──────────────────┤                                   │
│  │ id               │                                   │
│  │ organization_id  │                                   │
│  │ title            │                                   │
│  │ content          │                                   │
│  │ category         │                                   │
│  │ vector_id        │ (Reference to ChromaDB)           │
│  │ created_at       │                                   │
│  └──────────────────┘                                   │
│                                                          │
│  ┌──────────────────┐                                   │
│  │  subscriptions   │                                   │
│  ├──────────────────┤                                   │
│  │ id               │                                   │
│  │ organization_id  │                                   │
│  │ stripe_sub_id    │                                   │
│  │ plan_tier        │                                   │
│  │ status           │                                   │
│  │ current_period   │                                   │
│  └──────────────────┘                                   │
│                                                          │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│             VECTOR DATABASE (ChromaDB)                   │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  Collections (One per organization):                    │
│                                                          │
│  org_123_docs                                           │
│  ├── Document embeddings (1536 dims)                    │
│  ├── Document content                                   │
│  └── Metadata (title, category, source)                │
│                                                          │
│  org_456_docs                                           │
│  ├── Document embeddings                                │
│  ├── Document content                                   │
│  └── Metadata                                           │
│                                                          │
│  (Isolated per organization for security)              │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

---

### 5. External Services Integration

```
┌─────────────────────────────────────────────────────────┐
│              EXTERNAL SERVICES                           │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  OpenAI API                                             │
│  ├── text-embedding-3-small (Embeddings)               │
│  ├── gpt-4-turbo-preview (Main chat)                   │
│  └── gpt-3.5-turbo (Fallback/cost savings)             │
│                                                          │
│  Stripe                                                 │
│  ├── Checkout Sessions                                  │
│  ├── Subscriptions                                      │
│  ├── Webhooks                                           │
│  └── Customer Portal                                    │
│                                                          │
│  SendGrid/Postmark (Email)                             │
│  ├── Welcome emails                                     │
│  ├── Password reset                                     │
│  ├── Notifications                                      │
│  └── Billing updates                                    │
│                                                          │
│  Sentry (Monitoring)                                    │
│  ├── Error tracking                                     │
│  ├── Performance monitoring                             │
│  └── Alerts                                             │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

---

## Data Flow Examples

### Example 1: Customer Asks Question

```
Customer: "How do I reset my password?"
    │
    ▼
┌───────────────────────────────────────┐
│ 1. Frontend Widget                    │
│    • Capture input                    │
│    • Send via WebSocket               │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 2. Backend Receives Message           │
│    • Validate input                   │
│    • Check rate limits                │
│    • Authenticate organization        │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 3. Sentiment Analysis                 │
│    Input: "How do I reset my password?"│
│    Output: {                          │
│      label: "NEUTRAL",                │
│      score: 0.65,                     │
│      priority: "MEDIUM"               │
│    }                                  │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 4. Generate Embedding                 │
│    Input: "How do I reset my password?"│
│    Output: [0.234, -0.123, ..., 0.456]│
│    (1536 dimensions)                  │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 5. Vector Search (ChromaDB)           │
│    Query: [0.234, -0.123, ..., 0.456] │
│    Results:                           │
│      1. "Password Reset Guide" (0.91) │
│      2. "Account Settings" (0.73)     │
│      3. "Security FAQ" (0.68)         │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 6. Prepare Context                    │
│    Context = {                        │
│      retrieved_docs: [...],           │
│      conversation_history: [...],     │
│      user_message: "How do I...",     │
│      system_prompt: "You are..."      │
│    }                                  │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 7. LLM Generation (GPT-4)             │
│    Prompt: [Full context]             │
│    Response: "To reset your password, │
│    go to the login page and click     │
│    'Forgot Password'..."              │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 8. Store & Return                     │
│    • Save to database                 │
│    • Update analytics                 │
│    • Send via WebSocket               │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 9. Customer Sees Response             │
│    "To reset your password, go to..." │
│    [Helpful] [Not Helpful]            │
└───────────────────────────────────────┘

Total Time: 1-2 seconds
```

---

### Example 2: Admin Uploads Document

```
Admin: Uploads "Product Guide v2.pdf"
    │
    ▼
┌───────────────────────────────────────┐
│ 1. Upload via Dashboard               │
│    • File validation                  │
│    • Size check                       │
│    • Type verification                │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 2. Extract Text                       │
│    • Use pypdf library                │
│    • Clean text                       │
│    • Split into chunks                │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 3. Generate Embeddings (Batch)        │
│    Chunks: 15                         │
│    Embeddings: 15 vectors             │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 4. Store in ChromaDB                  │
│    Collection: org_123_docs           │
│    Documents: 15 chunks               │
│    Metadata: {                        │
│      title: "Product Guide v2",       │
│      category: "product",             │
│      source: "pdf"                    │
│    }                                  │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 5. Store Reference in Database        │
│    documents table:                   │
│    • id: doc_789                      │
│    • title: "Product Guide v2"        │
│    • organization_id: org_123         │
│    • vector_ids: [...]                │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 6. Success Message                    │
│    "Document uploaded successfully!"  │
│    Now available for chatbot queries  │
└───────────────────────────────────────┘
```

---

### Example 3: Subscription Payment

```
Customer: Clicks "Subscribe to Pro"
    │
    ▼
┌───────────────────────────────────────┐
│ 1. Stripe Checkout                    │
│    • Create checkout session          │
│    • Redirect to Stripe               │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 2. Customer Pays                      │
│    • Enters card details              │
│    • Stripe processes                 │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 3. Stripe Webhook                     │
│    Event: checkout.session.completed  │
│    Data: {                            │
│      customer_id: cus_123,            │
│      subscription_id: sub_456,        │
│      status: "active"                 │
│    }                                  │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 4. Backend Processes Webhook          │
│    • Verify signature                 │
│    • Update database                  │
│    • Activate subscription            │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 5. Send Confirmation Email            │
│    "Welcome to Pro! 🎉"               │
│    • Receipt                          │
│    • Getting started guide            │
└──────────┬────────────────────────────┘
           │
           ▼
┌───────────────────────────────────────┐
│ 6. Redirect to Dashboard              │
│    • Show Pro features                │
│    • Update UI                        │
└───────────────────────────────────────┘
```

---

## Security Architecture

```
┌─────────────────────────────────────────────────────────┐
│                   SECURITY LAYERS                        │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  Layer 1: Network                                       │
│  ├── HTTPS/TLS encryption                              │
│  ├── CORS policies                                      │
│  └── DDoS protection (via Cloudflare)                  │
│                                                          │
│  Layer 2: Authentication                                │
│  ├── JWT tokens (15 min expiry)                        │
│  ├── Refresh tokens (7 days)                           │
│  ├── API keys (for widget)                             │
│  └── Password hashing (bcrypt)                         │
│                                                          │
│  Layer 3: Authorization                                 │
│  ├── Role-based access control                         │
│  ├── Organization isolation                            │
│  ├── Resource ownership checks                          │
│  └── Rate limiting per user/org                        │
│                                                          │
│  Layer 4: Data                                          │
│  ├── Encrypted at rest (database)                      │
│  ├── Encrypted in transit (TLS)                        │
│  ├── Isolated vector stores                            │
│  └── Regular backups                                    │
│                                                          │
│  Layer 5: Application                                   │
│  ├── Input validation                                   │
│  ├── SQL injection prevention                          │
│  ├── XSS protection                                     │
│  ├── CSRF tokens                                        │
│  └── Error message sanitization                        │
│                                                          │
│  Layer 6: Monitoring                                    │
│  ├── Error tracking (Sentry)                           │
│  ├── Access logs                                        │
│  ├── Anomaly detection                                  │
│  └── Security alerts                                    │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

---

## Scalability Considerations

### Current Architecture (MVP - 100 users)

- Single server (Render)
- SQLite/PostgreSQL
- ChromaDB local instance

### Growth Phase (1,000 users)

- Load balancer
- Multiple app instances
- Managed PostgreSQL
- Redis caching
- ChromaDB with persistent storage

### Scale Phase (10,000+ users)

- Kubernetes cluster
- Database replicas
- Microservices (optional)
- CDN for static assets
- Message queue (Celery/RabbitMQ)

---

## Deployment Architecture

```
┌─────────────────────────────────────────────────────────┐
│                    PRODUCTION                            │
├─────────────────────────────────────────────────────────┤
│                                                          │
│  ┌────────────────────────────────────────────┐        │
│  │  Vercel/Netlify (Frontend)                 │        │
│  │  • Global CDN                              │        │
│  │  • Automatic scaling                        │        │
│  │  • SSL certificates                         │        │
│  └────────────────┬───────────────────────────┘        │
│                   │                                      │
│                   ▼                                      │
│  ┌────────────────────────────────────────────┐        │
│  │  Render (Backend)                          │        │
│  │  • Docker containers                        │        │
│  │  • Auto-deploy from git                     │        │
│  │  • Environment variables                    │        │
│  │  • Health checks                            │        │
│  └────────────────┬───────────────────────────┘        │
│                   │                                      │
│                   ├─────────────┬──────────────┐       │
│                   ▼             ▼              ▼        │
│          ┌─────────────┐  ┌──────────┐  ┌──────────┐  │
│          │ PostgreSQL  │  │ ChromaDB │  │  Redis   │  │
│          │  (Managed)  │  │  (Data)  │  │ (Cache)  │  │
│          └─────────────┘  └──────────┘  └──────────┘  │
│                                                          │
│  ┌────────────────────────────────────────────┐        │
│  │  Monitoring                                │        │
│  │  • Sentry (Errors)                         │        │
│  │  • Datadog/NewRelic (Performance)          │        │
│  │  • Uptime monitoring                       │        │
│  └────────────────────────────────────────────┘        │
│                                                          │
└─────────────────────────────────────────────────────────┘
```

---

## Technology Justification

| Tech              | Why?                                | Alternatives                   |
| ----------------- | ----------------------------------- | ------------------------------ |
| Flask             | Lightweight, Python, great for APIs | FastAPI, Django                |
| Vanilla JS        | No build step, fast loading         | React, Vue                     |
| ChromaDB          | Easy setup, Python native           | Pinecone, Weaviate             |
| SQLite→PostgreSQL | Simple dev, powerful prod           | MySQL, MongoDB                 |
| OpenAI            | Best-in-class LLMs                  | Anthropic Claude, local models |
| Stripe            | Industry standard payments          | PayPal, Paddle                 |
| Render            | Easy deployment, good free tier     | Heroku, AWS, GCP               |

---

This architecture is designed to:

- ✅ Start simple (MVP)
- ✅ Scale gradually
- ✅ Keep costs low initially
- ✅ Be maintainable by one developer
- ✅ Follow industry best practices

Ready to build? Start with `docs/PHASE_2_GUIDE.md`!
